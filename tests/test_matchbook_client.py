"""Unit tests for MatchbookClient."""
...
def _settings(dry_run=True, username="", password=""):
    s = MagicMock()
    s.dry_run = dry_run
    s.matchbook_username = username
    s.matchbook_password = password
...
def _client(dry_run=True, username="", password=""):
    return MatchbookClient(_settings(dry_run=dry_run, username=username, password=password))


def test_venue_property():
    c = _client()
    assert c.venue == Venue.MATCHBOOK


def test_place_order_dry_run_returns_id():
    c = _client(dry_run=True)
    m = Market(
        Venue.MATCHBOOK, "evt1/mkt1", "Test market",
        {"_runners": [{"id": "r1", "name": "yes"}, {"id": "r2", "name": "no"}]},
    )
    oid = c.place_order(m, "yes", 10.0, 0.55)
    assert oid.startswith("dry-matchbook-")
...
def test_best_quotes_no_credentials_returns_none():
    c = _client(username="", password="")
    m = Market(Venue.MATCHBOOK, "evt1/mkt1", "Test", {})
...
def test_is_binary(runners, expected):
    assert MatchbookClient._is_binary(runners) == expected
